...
<script lang="ts">
  ...
  import { ChevronLeft, Play, SkipForward, Square, Users, Check } from 'lucide-svelte';
  ...
  // One RPC per action, so the two-row change (close the current song, open the
  // next) is atomic and can never leave two songs playing.
  async function run(fn: 'start_show' | 'advance_show' | 'end_show', okMsg?: string) {
    if (busy) return;
    busy = true;
    try {
      const { error: e } = await supabase.rpc(fn, { p_party: partyId });
      if (e) {
  ...
  {:else}
    <div class="px-4 pb-32 flex flex-col gap-4">
      ...
      {:else if !isLive}
        <div class="bg-base-900 rounded-lg p-6 flex flex-col gap-4">
          ...
          <button on:click={() => run('start_show', '¡Arrancó el show!')} disabled={busy || !ordered.length}
      ...
      {:else}
        ...
        <div class="flex flex-col gap-2">
          <span class="text-xs uppercase tracking-widest text-cold-light">
            {upcoming.length ? 'Faltan ' + upcoming.length : 'No queda nada por tocar'}
          </span>
          {#if upcoming.length}
          ...
          <details class="rounded-lg bg-base-900 px-4 py-3">
            ...
            <ul class="mt-2 flex flex-col gap-1">
              {#each donePerfs as p (p.id)}
                <li class="text-sm text-cold-light/70 flex items-center gap-2">
                  <Check size={14} class="text-green-500 shrink-0" /> <span class="truncate">{songTitle(p)}</span>
                </li>
              {/each}
            </ul>
          </details>
    ...
    {#if isLive}
      <div class="fixed bottom-0 inset-x-0 bg-base-950/95 border-t border-base-900 p-4 flex items-center gap-3">
        <button on:click={() => run('advance_show')} disabled={busy}
        ...
        <button on:click={() => run('end_show', 'Show terminado.')} disabled={busy}
