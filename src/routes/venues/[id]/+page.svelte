<script lang="ts">
  ...
  let venue: any = null;
  // The embedded venue_contact row (#113). Null when the viewer has no reason to
  ...
  onMount(async () => {
    ...
    const [venueRes, adminRes, equipRes, partyRes, pendingRes] = await Promise.all([
      // venue_contact is embedded over its FK, so the exact address costs no extra
...
<div class="mt-2">
  ...
  {:else if venue}
    <div class="px-6 p-2 bg-base-900 rounded-lg shadow mx-4">
      ...
      {#if venue.area}<div class="mb-2 text-white">Zona: {venue.area}</div>{/if}
      <!-- The exact address comes from venue_contact and is simply absent when
           the viewer has no reason to have it (#113) — say so rather than
           rendering an empty field. -->
      {#if contact?.address}
        <div class="mb-2 text-white">Dirección: {contact.address}</div>
      {:else if venue.private}
        <div class="mb-2 text-cold-light text-sm">
          La dirección exacta se comparte con quienes organizan un toque aquí, quienes confirman asistencia y quienes van a tocar.
        </div>
      {/if}
      {#if contact?.contact_name}<div class="mb-2 text-cold-light">Persona de contacto: {contact.contact_name}</div>{/if}
      {#if contact?.contact}<div class="mb-2 text-cold-light">Contacto: {contact.contact}</div>{/if}
      {#if contact?.whatsapp}
        <div class="mb-2 text-cold-light">
          <a href="https://wa.me/{contact.whatsapp}" target="_blank" class="text-green-400 hover:underline flex flex-row items-center">
