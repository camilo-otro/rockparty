<script lang="ts">
  ...
  // Extracted from the /performance/[id] route so the SAME component can render
  // as a full page OR as an overlay on the party detail page (#94). The overlay
  // path is what keeps the party page mounted — and therefore keeps its state and
  // its realtime channel alive — instead of remounting it on every signup.
  export let performanceId: string | number;
  // Set only in overlay mode. Its presence is what swaps the VOLVER link for a
  // close action, since there is nothing to navigate back to.
...
<div class="mt-8">
  <div class="mb-4">
    <div class="flex flex-row items-center justify-between">
      {#if onClose}
        <button type="button" on:click={onClose} class="text-cold-light flex flex-row gap-2 mx-4 m-2"><ArrowLeft/> VOLVER</button>
      {:else}
